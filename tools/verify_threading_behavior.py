...
def test_threading_not_blocked():
 ...
 print("OK ChapterForge modules imported successfully")
 except Exception as e:
 print(f"FAIL Failed to import ChapterForge modules: {e}")
 return False
 ...
 print(f"OK Sorting completed: {len(sorted_files)} files")
 except Exception as e:
 print(f"FAIL Sorting failed: {e}")
 return False
 
 # Wait for background task and check if it completed normally
 bg_thread.join(timeout=2.0)
 if bg_thread.is_alive():
 print("FAIL Background task was blocked by sorting!")
 return False
 else:
 print("OK Background task completed normally during sorting")
 ...
 print(f"OK Probed {os.path.basename(filepath)}")
 except Exception as e:
 print(f"FAIL Failed to probe {os.path.basename(filepath)}: {e}")
 
 # Wait for background task
 bg_thread2.join(timeout=2.0)
 if bg_thread2.is_alive():
 print("FAIL Background task was blocked by file probing!")
 return False
 else:
 print("OK Background task completed normally during file probing")
 ...
 print(f"\nOK All threading tests passed in {total_time:.2f} seconds")
 return True
...
def main():
 ...
 print("\nFAIL FAILURE: ChapterForge may block other threads!")
