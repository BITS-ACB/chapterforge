...
### Importing and exporting chapter lists

- **File → Export Chapters…** saves the current list as **Audacity labels**, a
  **CUE sheet**, plain **timestamps**, or **Podcasting 2.0 JSON**.
- **File → Import Chapters…** (in edit mode) reads any of those formats and
  replaces the open master's markers; use **Save Changes** to keep them.

---

## 8. Auto-chapter by silence

**Tools → Auto-chapter by Silence…** analyses one long recording and proposes
...
## 9. Batch-building a library

**Tools → Batch Build Folder…** points ChapterForge at a *parent* folder and
...
A job file is a small, hand-editable text file that records the **order** and
**chapter titles** for a build (plus optional tags). Use it to script repeatable
builds or to prepare a build for the watcher.

- **Generate** one from the current folder: **File → Generate Job File…**
  (`Ctrl+G`).
- **Load** one to rebuild: **File → Load Job File…** (`Ctrl+L`).
...
## 12. Background folder watcher

ChapterForge can sit in the **system tray** and build new folders of MP3s for
you automatically.

- **Tools → Watch Folders…** (`Ctrl+W`) — define reusable *processes*: a watch
  folder plus a naming template for the output master.
- **Tools → Start Background Watcher** — minimises to the tray and watches.
- **Tools → Start Watcher at Sign-in** — registers a per-user startup entry so
  the watcher runs automatically when you log in.
- `chapterforge-cli --watch` runs the watcher standalone.
...
# Auto-chapter one long recording by its silent gaps
chapterforge-cli --split-silence recording.mp3 --noise-db -30 --min-silence 0.8

# Build from a job file
chapterforge-cli -j mybook.cfjob

# Run the tray watcher / check for updates
...
**Help → Check for Updates** queries GitHub Releases and tells you if a newer
